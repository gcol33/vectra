...
#define RAW_DT_BIT(dt) (1u << (uint32_t)(dt))

#define RAW_ACCEPTED_DTYPES (         \
    RAW_DT_BIT(TDC_DT_I8)  |          \
    RAW_DT_BIT(TDC_DT_I16) |          \
    RAW_DT_BIT(TDC_DT_I32) |          \
    RAW_DT_BIT(TDC_DT_I64) |          \
    RAW_DT_BIT(TDC_DT_U8)  |          \
    RAW_DT_BIT(TDC_DT_U16) |          \
    RAW_DT_BIT(TDC_DT_U32) |          \
    RAW_DT_BIT(TDC_DT_U64) |          \
    RAW_DT_BIT(TDC_DT_F32) |          \
    RAW_DT_BIT(TDC_DT_F64))

#define RAW_ACCEPTED_LAYOUTS (                          \
    (1u << (uint32_t)TDC_LAYOUT_VECTOR_1D) |            \
    (1u << (uint32_t)TDC_LAYOUT_RASTER_2D) |            \
    (1u << (uint32_t)TDC_LAYOUT_STACK_2D)  |            \
    (1u << (uint32_t)TDC_LAYOUT_VOLUME_3D))

static int raw_dtype_accepted(tdc_dtype dt) {
    return (RAW_ACCEPTED_DTYPES & RAW_DT_BIT(dt)) != 0u;
}

static int raw_layout_accepted(tdc_layout lo) {
    return (RAW_ACCEPTED_LAYOUTS & (1u << (uint32_t)lo)) != 0u;
}
