    ...
    switch (in_dtype) {
        case TDC_DT_I8: {
            for (size_t i = 0; i < n; ++i) {
                int8_t v;
                memcpy(&v, src + i, 1u);
                uint8_t z = zigzag_enc8(v);
                memcpy(dst->data + i, &z, 1u);
            }
            break;
        }
        case TDC_DT_I16: {
            for (size_t i = 0; i < n; ++i) {
                ...
                uint16_t z = zigzag_enc16(v);
                memcpy(dst->data + i * 2u, &z, 2u);
            }
            break;
        }
        case TDC_DT_I32: {
            for (size_t i = 0; i < n; ++i) {
                ...
                uint32_t z = zigzag_enc32(v);
                memcpy(dst->data + i * 4u, &z, 4u);
            }
            break;
        }
        case TDC_DT_I64: {
            for (size_t i = 0; i < n; ++i) {
                ...
                uint64_t z = zigzag_enc64(v);
                memcpy(dst->data + i * 8u, &z, 8u);
            }
            break;
        }
        default:
...
static tdc_status zigzag_decode(const uint8_t *src, size_t src_size,
    ...
    switch (in_dtype) {
        ...
        case TDC_DT_I16: {
            for (size_t i = 0; i < n; ++i) {
                uint16_t z;
                memcpy(&z, src + i * 2u, 2u);
                int16_t v = zigzag_dec16(z);
        ...
        case TDC_DT_I32: {
            for (size_t i = 0; i < n; ++i) {
                uint32_t z;
                memcpy(&z, src + i * 4u, 4u);
                int32_t v = zigzag_dec32(z);
        ...
        case TDC_DT_I64: {
            for (size_t i = 0; i < n; ++i) {
                uint64_t z;
                memcpy(&z, src + i * 8u, 8u);
                int64_t v = zigzag_dec64(z);
