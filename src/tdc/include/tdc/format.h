 ...
 *       18     2  entropy_id            (tdc_entropy_id)
 *       20     1  dtype                 (tdc_dtype)
 *       21     1  layout                (tdc_layout)
 *       22     1  rank                  (1..3)
 *       23     1  _reserved0
 *       24    24  dim[3]                (int64)
 *       48     8  uncompressed_size     (residual bytes BEFORE entropy stage)
 *       56     4  side_meta_size        (model side metadata)
 *       60     4  payload_size          (entropy-compressed payload bytes)
 *       64     4  xform_params_size     (TLV section size; 0 if none)
 *       68     4  validity_size         (validity bitmap byte count; 0 if none)
 *       72     8  _reserved1            (must be 0)
 *       --   ---  total = 80 bytes
 *
