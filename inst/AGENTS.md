...
- `create_index(path, column, ci = FALSE)` writes a `.vtri` sidecar; pass a character vector (in any order) for composite indexes. `has_index(path, column)` reports whether one exists and is usable against the store as it now stands. `append_vtr(along = "rows")` leaves each sidecar valid, taking in the appended row groups rather than rebuilding from the whole store. An index that stops matching its store, or that cannot be read at all, is ignored rather than used to prune — it only ever saves a scan work, so losing one costs speed and never rows — so rebuild it with `create_index()` when `has_index()` turns `FALSE`. `explain()` names the index a scan will probe. A lookup costs the part of the index it touches rather than the whole file: a sidecar past a few MB is mapped read-only rather than read into memory, so an index far larger than RAM stays usable and a fetch does not pay for the index's size. Building one is bounded the same way — the entries are sorted through the streaming budget (`vectra_mem()`) and written in one pass — so an index over a store larger than memory costs disk rather than RAM. Sidecars written by vectra 0.11.8 and earlier read as absent; `create_index()` rebuilds them.
