...
When the projection is itself a stack of environmental rasters, `mop()` folds
the same distance over the projection grid and adds the strict-extrapolation
layers of `mop::mop(type = "detailed")`. Per cell it reports how many predictors
fall below their calibration minimum (`towards_low`) or above their maximum
(`towards_high`), their sum (`mop_simple`), and a binary novelty flag
(`mop_basic`), alongside the continuous `mop_distance` surface. The projection
raster streams one tile-row strip at a time and the result is written aligned to
its grid, so the whole surface is computed out-of-core.
...
* To judge whether a projection is within the calibrated conditions, score it
  with `feature_knn()`, or `mop()` for a full raster novelty surface.
```
