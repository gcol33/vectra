...
Each operation states the memory it needs to run. The three tiers below group
the spatial verbs by how much a run keeps resident.
...
- **All-to-all.** Inherently resident: `spatial_overlay()` and operations such
  ...
  collect the result and run sf or terra on it.

The streaming envelope covers every operation whose memory can be bounded. For
the rest, vectra collects the result and hands it to sf or terra.

## Where to go next

- [Streaming spatial operations](streaming-spatial.html) for the hands-on
  companion: every vector verb run end to end on a real layer, with runnable
  code and figures.
