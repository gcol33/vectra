...
similar). Both work in `filter()` and `mutate()`:
...
`dl_dist()` counts transpositions as a single operation (Damerau-Levenshtein),
useful for catching common typos:
...
`resolve(fk, pk, value)` is a scalar self-join — it looks up `value` where
`pk == fk` within the same batch, denormalising a parent-child table without
an explicit join:
...
`propagate(parent_id, id, seed)` walks the tree and fills every node with
the nearest non-NA ancestor value of `seed`:
...
`append_vtr()` adds new rows as a row group without rewriting existing data:
...
`delete_vtr()` marks rows as deleted via a tombstone side file — the `.vtr`
...
`diff_vtr()` computes a key-based logical diff between two snapshots:
