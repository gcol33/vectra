...
| `mutate(...)` | yes | Arithmetic, comparison, boolean, `is.na()` expressions |
...
Version 2 adds per-column annotation strings after each column's name and type
byte. This enables roundtripping of Date, POSIXct, and factor columns. Version
1 files are read normally (no annotations). Writing annotations automatically
selects version 2.

## explain() contract

`explain()` prints the plan tree without executing it. The output shows:
...
- **No nested expressions in aggregation**: `summarise(m = mean(x + y))` does
  not work. Use `mutate(xy = x + y) |> summarise(m = mean(xy))` instead.
- **No string operations**: No `substr()`, `paste()`, `grepl()`, etc. in
  expressions. Use `collect()` first, then manipulate in R.
...
- **distinct with .keep_all**: Falls back to R when `.keep_all = TRUE` with a
  column subset.
- **rank() and dense_rank() are O(n^2)**: Within each group partition, ranking
  compares every pair of values. Fine for typical group sizes (thousands of
  rows), but will be slow for very large ungrouped rankings. Use
  `arrange() |> mutate(rn = row_number())` for large-scale ranking.
- **Single-threaded**: The engine uses one thread. Parallelism is not planned.
- **No predicate pushdown**: Filters are not pushed into scan nodes.
  Row-group-level pruning is not implemented.
- **No column pruning**: All columns in a row group are read from disk, even if
