...
- **distinct with .keep_all**: Falls back to R when `.keep_all = TRUE` with a
  column subset.
- **Limited string operations**: `nchar()`, `substr()`, `grepl()` (fixed match only), `tolower()`, `toupper()`, `trimws()`, `paste0()` (2 args), `gsub()`/`sub()` (fixed patterns), `startsWith()`, `endsWith()` are supported. No regex or multi-argument `paste()`. Use `collect()` first for unsupported string operations.
