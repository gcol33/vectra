...
| `mutate(...)` | yes | Arithmetic, comparison, boolean, `is.na()`, `nchar()`, `substr()`, `grepl()`, math (`abs`, `sqrt`, `log`, `exp`, `floor`, `ceiling`, `round`), `if_else()`, `between()`, `%in%`, type casting (`as.numeric`), `tolower()`, `toupper()`, `trimws()` |
...
- **Limited string operations**: `nchar()`, `substr()`, `grepl()` (fixed match
  only), `tolower()`, `toupper()`, `trimws()` are supported. No regex or
  `paste()`. Use `collect()` first for unsupported string operations.
