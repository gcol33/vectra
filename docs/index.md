...
rebuilding the pipeline. With a `by` key it instead splits the query
into per-key shards on disk, turning a model that couples within a group
into a set of independent per-shard fits.
[`group_map()`](https://gillescolling.com/vectra/reference/group_map.md)
runs a function on each shard and returns the results keyed by shard;
